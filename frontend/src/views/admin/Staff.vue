      ...
      <el-dialog title="提示" v-model="dialogVisible" width="30%">
        <el-form :model="form" label-width="120px">
          ...
          <el-form-item label="部门">
            <el-input v-model="form.departmentName" style="width: 80%"></el-input>
          </el-form-item>
        </el-form>
        <template #footer>
...
export default {
  ...
  methods: {
    ...
    updateTotal() {
      request.get("/admin/staff/count").then(res => {
        this.total = res.data;
      })
    },
    load() {
    ...
    save() {
      if (this.form.id) {   //update
        request.put("/admin/staff/" + this.form.id, this.form).then(res => {
          console.log(res);
          if (res.code === '0') {
            this.$message({
              type: "success",
        ...
        request.post("/admin/staff", this.form).then(res => {
          console.log(res)
          if (res.code === '0') {
            this.$message({
              type: "success",
    ...
    handleDelete(id) {
      console.log(id)
      request.delete("/admin/staff/" + id).then(res => {
        if (res.code === '0') {
          this.$message({
            type: "success",
            message: "删除成功"
