...
func prefixSpan(db []Sequence, minSupport int, pattern Sequence, patternSupport int) ([]Sequence, []int) {
	items := frequentItems(db, minSupport) // 寻找当前db中长度为1的频繁序列

	var patterns []Sequence // 序列结果
	var freCnts []int       // 序列次数
	if len(pattern) > 0 {
		patterns = append(patterns, pattern) // 当前序列
		freCnts = append(freCnts, patternSupport)
	}
	...
	for _, item := range items {
		...
		if support >= minSupport {                                   // 大小满足最小支持度
			...
			newPatterns, newFreCnts := prefixSpan(projected, minSupport, superPattern, support)
			patterns = append(patterns, newPatterns...)
			freCnts = append(freCnts, newFreCnts...)
		}
	}

	return patterns, freCnts
}

func PrefixSpan(db []Sequence, minSupport int) ([]Sequence, []int) {
